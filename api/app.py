...
# @cross_origin(allow_headers = ['Content-Type'])# @login_required
...
@app.route('/newEmployee', methods=['POST'])  # @login_required
@cross_origin(allow_headers=['Content-Type'])
def addemployee():
  data = request.get_json()


# birth_date = Strip the time!!!!!!!!#birthdate = datetime.datetime.strptime(data['birth_date'], '%Y-%M-%d')
...
@app.route('/view/', methods=['GET'])  # @login_required# @cross_origin('*')
def viewEmployee():
...
@app.route('/viewDeactivated/', methods=['GET', 'POST'])  # @login_required
@cross_origin('*')
...
@app.route('/search/', methods=['GET', 'POST'])  # @login_required
@cross_origin('*')
def searchEmployee():
  data = request.get_json()
...
if employee:  # 1
for active  # 0
for inactive  # change the status to 0
...
if employee is None:
  ...
  try:  # Check
if the jsondata is empty, can be done here or front end.
if data['fname'] == ''
or data['fname'] is None:
...
employees = []
if summary is None:  # return jsonify({
  "message": "No data to show"
})
for employee in summary:
  employee_data={}
...
if new_time is None:
  ...
  try: #new morning log time# morning time in start
if morning1 == '':
  new_time.morning_time_in_start = new_time.morning_time_in_start
else :
  new_time.morning_time_in_start = morning1# morning time out start
if morning2 == '':
  new_time.morning_time_out_start = new_time.morning_time_out_start
else :
  new_time.morning_time_out_start = morning2# morning time out end
if morning3 == '':
  new_time.morning_time_out_end = new_time.morning_time_out_end
else :
  new_time.morning_time_out_end = morning3# new afternoon log time# afternoon time in start
if after1 == '':
  new_time.afternoon_time_in_start = new_time.afternoon_time_in_start
else :
  new_time.afternoon_time_in_start = after1# afternoon time out start
if after2 == '':
  new_time.afternoon_time_out_start = new_time.afternoon_time_out_start
else :
  new_time.afternoon_time_out_start = after2# afternoon time out end
if after3 == '':
...
if employee is None:
  ...
  atts = Attendance.query.filter(and_(Attendance.employeeid == empID, Attendance.date == str(datenow))).order_by(Attendance.date.desc()).first()# ////////////////////////////IF ID IS NOT LISTED IN THE ATTENDACE CRETAE NEW///////////////////////////////#
if atts is None:
...
dbase.session.commit()
employee1 = Overtime.query.filter(and_(Overtime.employeeid == empID, Overtime.overtimeStatus == 1, Overtime.overtimeDate == datenow2)).first()
...
return jsonify({
  'message': 'you cannot time in twice'
})
elif atts.morningStatus == 1 and atts.afterStatus == 0:
  print 'ccccccccc'
return jsonify({
  'message': 'no time out at this time'
})
...
print 'ddddddddddddd'
return jsonify({
  'message': 'not late, kindly dont forget to timeout in morning'
})
else :
...
atts.morningTimeIn = datetime.now()# atts.morningRemark = wala pa nabutang
print 'ggggggggggg'
dbase.session.commit()
return jsonify({
  'message': 'late'
...
atts.morningTimeOut = datetime.now()# atts.morningRemark = wala pa nabutang
print 'iiiiiiiiiiii'
dbase.session.commit()
return jsonify({
  'message': 'time out'
...
dbase.session.commit()
return jsonify({
  'message': 'late, kindly dont forget to timeout in morning'
})
else :
...
elif(now > m12) and(now <= a1): #12 - 7 pm
if atts.morningStatus == 0 and atts.afterStatus == 0:
  if atts.afterTimeOut is None:
  atts.afterStatus = 1
atts.afterDailyStatus = 'not late'
atts.afterTimeIn = datetime.now()# atts.morningRemark = wala pa nabutang
print 'ooooooooooo'
dbase.session.commit()
print 'ppppppppppppppp'
return jsonify({
  'message': 'time in for afternoon'
})
else :
...
return jsonify({
  'message': 'no time out for afternoon at this time'
})
elif atts.morningStatus == 1 and atts.afterStatus == 1:
  atts.morningStatus = 0
atts.morningTimeOut = datetime.now()
...
dbase.session.commit()
return jsonify({
  'message': 'no time in for this time'
})
...
absents()
return jsonify({
  'message': 'not time in for afternoon'
})
...
absents()
return jsonify({
  'message': 'time out for afternoon'
})
...
absents()
return jsonify({
  'message': 'time out for afternoon and morning'
})
...
atts = Attendance.query.filter(and_(Attendance.employeeid == empID, Attendance.date == datenow)).order_by(
  Attendance.date.desc()).first()# dbase.session.commit()# date1 = atts.date
print "second"
dates = Attendance.query.filter_by(date = datenow).first()
employee1 = Overtime.query.filter(and_(Overtime.employeeid == empID, Overtime.overtimeStatus == 1, Overtime.overtimeDate == datenow2)).first()
...
atts = Attendance.query.filter(
  ...
  Attendance.date.desc()).first()# ///////////////////////////////////////////CHECK IF THE DATE IS SAME//////////////////////////////////////
...
dbase.session.commit()
return jsonify({
  'message': '"time in for afternoon." (time out for morning next time,) '
})
else :
...
return jsonify({
  ...
  return jsonify({
    'message': 'Request for overtime '
  })
else :
  return jsonify({
    'message': 'Please request overtime first!'
  })
else :
...
def set_time():
  data = request.get_json()# ako ra edit aning line sa ubos
...
atts = Attendance.query.filter(and_(Attendance.employeeid == empID, Attendance.date == str(
  datenow))).order_by(Attendance.date.desc()).first()# ////////////////////////////IF ID IS NOT LISTED IN THE ATTENDACE CRETAE NEW///////////////////////////////#
if not atts:
...
atts = Attendance.query.filter(
  and_(Attendance.employeeid == empID, Attendance.date == datenow)).order_by(
  Attendance.date.desc()).first()# ///////////////////////////////////////////CHECK IF THE DATE IS SAME//////////////////////////////////////
if atts.date == datenow: # ////////////////////////IF DATE IS SAME////////////////////////////#
  print atts.date + 'nabuang siya'
if now >= m7 and now <= m9:
  if atts.morningStatus == 0 and atts.afterStatus == 0:
  if atts.morningTimeOut is None:
...
absent = []
for i in employees: #print "this" + str(i)
for j in presents:
  if i == j:
  pass
else :
...
if overtime:
  return jsonify({
    'message': 'Request already sent, Please wait for the admin to approve'
  })
else :
...
return jsonify({
  ...
  return jsonify({
    'message': 'Date requested is not valid'
  })

@app.route('/view/overtime/requests', methods = ['GET'])# @login_required
@cross_origin(allow_headers = ['Content-Type'])
def view_requests():
...
@app.route('/approve/request', methods = ['POST'])# @login_required
@cross_origin(allow_headers = ['Content-Type'])
def approve():
  data = request.get_json()
...
dbase.session.commit()
return jsonify({
  'message': 'Overtime approved successfuly!'
})

@app.route('/decline/request', methods = ['POST'])# @login_required# @cross_origin(allow_headers = ['Content-Type'])
def decline():
  data = request.get_json()
...
dbase.session.commit()
return jsonify({
  'message': 'Overtime declined successfuly!'
})
...
@app.route('/view/admin/logs', methods = ['GET'])# @login_required# @cross_origin("*")
def view_logs():
  log = Logs.query.all()
if not log:
  return jsonify({
...
# @app.route('/unseencounter', methods = ['GET'])# def counterunseen(): #log = Logs.query.count(counter = 1)# if not log: #return jsonify({
  'message': 'No logs to show'
})#
else :#return jsonify({
  'adminlogs': log
...
def otTimeout():
  nowtime = datetime.now().strftime("%H%M")# nowdate = datetime.now().strftime("%Y-%d-%m")
timeout = "2200"
timeout1 = "2300"
