...
def newAdmin():
    ...
    try:
        ...
        if data['code'] == '':
            ...
            new.code = generate_password_hash(data['code'],
                                              method='sha256')
        dbase.session.commit()
...
def searchEmployee():
    ...
    notactive = []
    activate = Employee.query.filter(and_(Employee.lname == employee1,
                                          Employee.employeestatus == 0)).all()
...
def delEmployee():
    ...
    employee = Employee.query.filter(and_(Employee.code == data['code'
                                                                ], Employee.employeestatus == 1)).first()
...
def ReActEmployee():
    ...
    employee = Employee.query.filter(and_(Employee.code == data['code'
                                                                ], Employee.employeestatus == 0)).first()
...
def company_month(dates):
    ...
    summary = Attendance.query.filter(extract('year', Attendance.date)
                                      == dates.strftime('%Y')).filter(extract('month',
                                                                              Attendance.date) == dates.strftime('%m')).all()
    employees = []
    for employee in summary:
        employee_data = {}
        name = \
            Employee.query.filter_by(employeeid=employee.employeeid).first()
        overtimee = \
            Overtime.query.filter_by(employeeid=employee.employeeid).order_by(
                Overtime.overtimeDate.desc()).first()
        employee_data['name'] = name.fname + ' ' + name.mname + ' ' \
            + name.lname
        ...
        employee_data['morningDailyStatus'] = \
            employee.morningDailyStatus
...
def company_week(sort_date):
    ...
    print year
    print week_number
    summary = Attendance.query.filter(Attendance.week_number
                                      == week_number).filter(extract('year', Attendance.date)
                                                             == year).order_by(Attendance.week_number.desc()).all()
    employees = []
    if summary is None:
...
def employee_monthly(emp_id):
    summary = Attendance.query.filter(Attendance.employeeid
                                      == emp_id).order_by(Attendance.date.desc()).all()
    employees = []
    for employee in summary:
        employee_data = {}
        name = \
...
@app.route('/employee_summary/weekly/<string:dates>/<int:emp_id>',
           methods=['GET'])
def employee_week(dates, emp_id):
    ...
    print year
    print week_number
    summary = Attendance.query.filter(and_(Attendance.employeeid
                                           == emp_id, Attendance.week_number == week_number,
...
def edit_time():
    ...
    morning1 = dt.datetime.strptime(data['morning_time_in_start'],
                                    '%H:%M')
    morning2 = dt.datetime.strptime(data['morning_time_out_start'],
                                    '%H:%M')
    morning3 = dt.datetime.strptime(data['morning_time_out_end'],
                                    '%H:%M')
    after1 = dt.datetime.strptime(data['afternoon_time_in_start'],
                                  '%H:%M')
    after2 = dt.datetime.strptime(data['afternoon_time_out_start'],
                                  '%H:%M')
    after3 = dt.datetime.strptime(data['afternoon_time_out_end'],
                                  '%H:%M')
    ...
    if new_time is None:
        ...
        try:
           ...
           # morning time in start
            ...
            if morning1 == '':
                new_time.morning_time_in_start = \
                    new_time.morning_time_in_start
            else:
           ...
           # morning time out start
            ...
            if morning2 == '':
                new_time.morning_time_out_start = \
                    new_time.morning_time_out_start
            else:
           ...
           # afternoon time in start
            ...
            if after1 == '':
                new_time.afternoon_time_in_start = \
                    new_time.afternoon_time_in_start
            else:
           ...
           # afternoon time out start
            ...
            if after2 == '':
                new_time.afternoon_time_out_start = \
                    new_time.afternoon_time_out_start
            else:
           ...
           # afternoon time out end
            ...
            if after3 == '':
                new_time.afternoon_time_out_end = \
                    new_time.afternoon_time_out_end
            else:
...
def timein():
   ...
   # 1 for active and 0 for inactive
    ...
    PersonalTimeIN = \
        PersonalTime.query.filter(and_(PersonalTime.employeeid
                                       == empID, PersonalTime.date
                                       == str(datenow))).order_by(PersonalTime.date.desc()).first()
    ...
    if employee is None:
        ...
        if PersonalTimeIN is None:
            ...
            atts = Attendance.query.filter(and_(Attendance.employeeid
                                                == empID, Attendance.date
                                                == str(datenow))).order_by(Attendance.date.desc()).first()
         ...
         # ////////////////////////////IF ID IS NOT LISTED IN THE ATTENDACE CRETAE NEW///////////////////////////////#
            ...
            if atts is None:
                ...
                atts = \
                    Attendance.query.filter_by(employeeid=empID).order_by(
                ...
                dbase.session.commit()
                employee1 = \
                    Overtime.query.filter(and_(Overtime.employeeid
                                               == empID, Overtime.overtimeStatus == 1,
                                               Overtime.overtimeDate == datenow2)).first()
               ...
               # nowdate = atts.date
                ...
                if now >= m7 and now <= m9:
                    if atts.morningStatus == 0 and atts.afterStatus \
                            == 0:
                    ...
                    elif atts.morningStatus == 1 and atts.afterStatus \
                            == 0:
                        print 'ccccccccc'
                        return jsonify({'message': 'no time out at this time'
                                        })
                    elif atts.morningStatus == 0 and atts.afterStatus \
                            == 1:
                        if atts.morningTimeOut is None:
                            atts.afterStatus = 0
                            atts.morningStatus = 1
             ...
             # ////////// ///////////////////////////////IF ID IS EXISTING/////////////////////////////////////////////////////////
                ...
                dates = \
                    Attendance.query.filter_by(date=datenow).order_by(
                        Attendance.date.desc()).first()
                print dates
                atts = \
                    Attendance.query.filter(and_(Attendance.employeeid
                                                 == empID, Attendance.date
                ...
                if atts.date == datenow:
                  ...
                  # ////////////////////////IF DATE IS SAME////////////////////////////#
                    ...
                    if now >= m7 and now <= m9:
                        ...
                        elif atts.morningStatus == 1 \
                                and atts.afterStatus == 0:
                            print 'CCCCCCCCCCCCCCCC'
                            return jsonify({'message': 'no time out at this time'
                                            })
                        elif atts.morningStatus == 0 \
                                and atts.afterStatus == 1:
                            if atts.morningTimeOut is None:
                                atts.afterStatus = 0
                                atts.morningStatus = 1
                                atts.afterTimeOut = datetime.now()
...
def set_time():
   ...
   # ako ra edit aning line sa ubos
    ...
    personal_time = PersonalTime(
        employeeid=data['id'],
        morningTimeIn=data['morningIn'],
        morningTimeOut=data['morningOut'],
        morningTimeOutend=data['morningOutend'],
        afterTimeIn=data['afterIn'],
        afterTimeOut=data['afterOut'],
        afterTimeOutend=data['afterOutend'],
        date=data['date'],
    )
...
def personalTime_in(
   ...
   # 1 for active and 0 for inactive
    ...
    attendancenNew = Attendance(employeeid=empID)
    employee = Employee.query.filter(and_(Employee.employeeid == empID,
                                          Employee.employeestatus == 1)).first()
    if not employee:
        return jsonify({'message': 'user not found'})
    atts = Attendance.query.filter(and_(Attendance.employeeid == empID,
                                        Attendance.date
                                        == str(datenow))).order_by(Attendance.date.desc()).first()
   ...
   # ////////////////////////////IF ID IS NOT LISTED IN THE ATTENDACE CRETAE NEW///////////////////////////////#
    ...
    if not atts:
        ...
        atts.week_number = week_no
        dbase.session.commit()
        employee1 = Overtime.query.filter(and_(Overtime.employeeid
                                               == empID, Overtime.overtimeStatus == 1,
                                               Overtime.overtimeDate == datenow2)).first()
        dates = Attendance.query.filter_by(date=datenow).first()
        if dates:
...
def absents():
    ...
    present = Attendance.query.filter(and_(Attendance.date == datenow1,
                                           Attendance.absentTotal == 0)).all()
    presents = []
    ...
    if present:
        ...
        for i in absent:
            v = Attendance.query.filter(and_(Attendance.employeeid
                                             == i, Attendance.absentTotal == None)).first()
...
def add_remarks(codes):
    ...
    print ids
    remarks = \
        Attendance.query.filter_by(employeeid=ids.employeeid).order_by(
            Attendance.date.desc()).first()
...
def auto_TimeOut():
    timeout = Overtime.query.filter(and_(Overtime.overtimeid == 1,
                                         Overtime.overtimeDate
                                         == lgdate).order_by(Overtime.overtimeDate.desc())).all()
    if timeout is None:
        pass
    else:
        for i in timeout:
...
def request_overtime():
    ...
    overtime = Overtime.query.filter(and_(Overtime.employeeid
                                          == employee.employeeid, Overtime.overtimeStatus == 1,
                                          Overtime.overtimeDate == data['dates'])).first()
    if overtime:
...
def approve():
    data = request.get_json()
    overtime = Overtime.query.filter(and_(Overtime.overtimeStatus == 0,
                                          Overtime.employeeid == int(data['id']))).first()
    ...
    if overtime is None:
        ...
        msg = name.fname + ' ' + name.lname \
            + ' overtime request has been approved'
        logmessage = Logs(details=msg, log_date=lgdate)
        dbase.session.add(logmessage)
        dbase.session.commit()
        logsid = \
            Logs.query.filter_by(logStatus=0).order_by(
                desc(Logs.logID)).first()
        logsid.logStatus = 1
        logsid.counter = 1
        dbase.session.commit()
...
def decline():
    data = request.get_json()
    overtime = Overtime.query.filter(and_(Overtime.overtimeStatus == 0,
                                          Overtime.employeeid == int(data['id'
...
def notifications():
    log = Logs.query.filter(and_(Logs.logStatus == 1, Logs.counter
                                 == 1)).all()
    count = 0
    if not log:
